...
export default function CekInvoicePage() {
  ...
  const getItemPhotos = (item: InvoiceItem) => {
    ...
    if (item.arrived_photo_url) {
      photos.push({ title: 'Foto Bukti Tiba di Lokasi', url: item.arrived_photo_url })
    }
    if (item.handover_photo_url) {
      photos.push({ title: 'Foto Bukti Serah Terima Paket', url: item.handover_photo_url })
    }

    // Digital Signatures
    if (item.signature_photo_url) {
      photos.push({ title: 'Tanda Tangan Digital Apoteker', url: item.signature_photo_url })
    }
    if (item.pod_signature_photo_url) {
      photos.push({ title: 'Tanda Tangan Digital POD (Gudang K-24)', url: item.pod_signature_photo_url })
    }
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto py-1">
                      {photos.length === 0 ? (
                        ...
                        photos.map((p, pIdx) => (
                          ...
                          >
                            <img
                              src={p.url.startsWith('data:image') || p.url.startsWith('http') ? p.url : `/api/${p.url.replace(/^\//, '')}`}
                              alt={p.title}
              ...
              {getItemPhotos(selectedInvoice).length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {getItemPhotos(selectedInvoice).map((photo, pIdx) => {
                    const src = photo.url.startsWith('data:image') || photo.url.startsWith('http')
                      ? photo.url
                      : `/api/${photo.url.replace(/^\//, '')}`
      ...
      {lightboxImage && (
        ...
        >
          ...
          <img
            src={lightboxImage.startsWith('data:image') || lightboxImage.startsWith('http') ? lightboxImage : `/api/${lightboxImage.replace(/^\//, '')}`}
