...
import { Search, AlertCircle, Check, X, FileText, Eye, Pencil, ShieldAlert, Trash2, Play, ChevronDown, MoreVertical, KeyRound, Copy, EyeOff, RefreshCw } from 'lucide-react'
...
function DriversPageContent() {
  ...
  // Reset Password Driver Modal State
  ...
  const openResetPasswordModal = (driver: Driver) => {
    setNewPassword('')
    setShowPassword(false)
    setResetPasswordModal({ isOpen: true, driver, loading: false })
  }
  ...
  const handleSaveResetPassword = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!resetPasswordModal.driver) return
    ...
    try {
      await adminAPI.resetDriverPassword(resetPasswordModal.driver.id, newPassword)
      toast.success(`Password akun driver "${resetPasswordModal.driver.name}" berhasil di-reset!`)
      setResetPasswordModal({ isOpen: false, driver: null, loading: false })
    } catch (err: any) {
      toast.error(err.response?.data?.message || 'Gagal mereset password driver.')
    } finally {
  ...
  // Reset Password Driver Modal UI
  const renderResetPasswordModal = () => {
    if (!resetPasswordModal.isOpen || !resetPasswordModal.driver) return null

    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        <div className="bg-card w-full max-w-md rounded-2xl border border-border overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
          <div className="flex justify-between items-center px-6 py-4 border-b border-border bg-purple-50 dark:bg-purple-950/40">
            <div className="flex items-center gap-2">
              <KeyRound className="h-5 w-5 text-purple-600 dark:text-purple-400" />
              <h3 className="font-bold text-foreground text-sm">Reset Password Driver</h3>
            </div>
            <button
              onClick={() => setResetPasswordModal({ isOpen: false, driver: null, loading: false })}
          ...
          <form onSubmit={handleSaveResetPassword} className="p-6 space-y-4">
            <div className="p-3 bg-muted/40 rounded-xl border border-border space-y-1">
              <div className="text-xs text-muted-foreground font-semibold">Driver Terkait:</div>
              <div className="font-bold text-sm text-foreground">{resetPasswordModal.driver.name}</div>
              <div className="text-xs text-muted-foreground flex items-center gap-2 font-mono">
                <span>@{resetPasswordModal.driver.username}</span>
                <span>•</span>
                <span>{resetPasswordModal.driver.email}</span>
              </div>
            </div>

            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-xs font-bold text-foreground">Password Baru Driver</label>
                <button
                  type="button"
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-input bg-background pl-3.5 pr-10 py-2.5 text-sm font-mono outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20"
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                Password baru akan langsung aktif dan dapat digunakan driver untuk login ke aplikasi.
              </p>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-border">
              <button
                ...
                className="px-4 py-2 text-xs font-semibold rounded-xl border border-border bg-background hover:bg-accent text-foreground transition-colors disabled:opacity-50 cursor-pointer"
              >
                Batal
              </button>
              <button
                type="submit"
                disabled={resetPasswordModal.loading}
                className="inline-flex items-center justify-center px-4 py-2 text-xs font-bold rounded-xl bg-purple-600 hover:bg-purple-700 text-white transition-colors disabled:opacity-50 min-w-[120px] shadow-sm cursor-pointer"
              >
